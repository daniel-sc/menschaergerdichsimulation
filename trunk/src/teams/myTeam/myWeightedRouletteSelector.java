 ...
 * 
 * changed from original version: Use of java.util.Map instead of gnu.trove.*
 *
 * @author Neil Rotstan
 * @author Klaus Meffert
 * @since 1.0
 */
public class myWeightedRouletteSelector
...
/** String containing the CVS revision. Read out via reflection!*/
   ...
   */
  private HashMap<IChromosome, SlotCounter> m_wheel = new HashMap<IChromosome, SlotCounter>();
   ...
   * @since 2.0
   */
  public myWeightedRouletteSelector()
      throws InvalidConfigurationException {
   ...
   * @since 3.0
   */
  public myWeightedRouletteSelector(Configuration a_config)
      throws InvalidConfigurationException {
    super(a_config);
  ...
  protected synchronized void add(final IChromosome a_chromosomeToAdd) {
    ...
    // -----------------------------------------------------------------
    SlotCounter counter = m_wheel.get(a_chromosomeToAdd);
    if (counter != null) {
  ...
  public synchronized void selectChromosomes(int a_howManyToSelect,
      Population a_to_pop) {
	  System.out.println("selectChromosomes: from_pop="+m_wheel.size() +" how many="+a_howManyToSelect);
    ...
    // -------------------------------------------------------------------
    Set<Entry<IChromosome, SlotCounter>> entries = m_wheel.entrySet();
    ...
    Iterator<Entry<IChromosome, SlotCounter>> entryIterator = entries.iterator();
    for (int i = 0; i < numberOfEntries; i++) {
      Map.Entry<IChromosome, SlotCounter> chromosomeEntry = entryIterator.next();
      IChromosome currentChromosome = chromosomeEntry.getKey();
      SlotCounter currentCounter = chromosomeEntry.getValue();
      fitnessValues[i] = currentCounter.getFitnessValue();
      counterValues[i] = fitnessValues[i] * currentCounter.getCounterValue();
      chromosomes[i] = currentChromosome;
      if(fitnessValues[i]<0.0d)
    	  System.err.println("keine fitness!");
  ...
  private void scaleFitnessValues() {
    ...
    Iterator<SlotCounter> counterIterator = m_wheel.values().iterator();
    while (counterIterator.hasNext()) {
      SlotCounter counter = counterIterator.next();
...
public Object clone() {
    try {
      myWeightedRouletteSelector result = new myWeightedRouletteSelector(
          getConfiguration());
      result.m_wheel = (HashMap<IChromosome, SlotCounter>) m_wheel.clone();
  ...
  public boolean equals(Object o) {
    myWeightedRouletteSelector other = (myWeightedRouletteSelector) o;
    if (other == null) {
      return false;
    }
